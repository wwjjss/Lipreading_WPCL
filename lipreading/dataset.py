...
class MyDataset(object):
    def __init__(self, modality, data_partition, data_dir, label_fp, annonation_direc=None,
        ...
        assert os.path.isfile(label_fp), "File path provided for the labels does not exist. Path iput: {}".format(
            label_fp)
    ...
    def _get_files_for_partition(self):
        ...
        self._data_files = [f for f in self._data_files if
                            f.replace("/", "\\").split('\\')[self.label_idx] in self._labels]
    ...
    # 可变长度增强
    def _apply_variable_length_aug(self, filename: object, raw_data: object) -> object:
        # 读取txt文件查看单词持续时间用于裁剪
        info_txt = os.path.join(self._annonation_direc,
                                *filename.replace("/", "\\").split('\\')[self.label_idx:])  # swap base folder
        info_txt = os.path.splitext(info_txt)[0] + '.txt'  # swap extension 分离文件名和扩展名
        info = read_txt_lines(info_txt)

        utterance_duration = float(info[4].split(' ')[1])  # 话语时长29帧
        half_interval = int(utterance_duration / 2.0 * self.fps)  # num frames of utterance / 2  一半话语时长

        n_frames = raw_data.shape[0]  # shape[0]取第一维
        mid_idx = (n_frames - 1) // 2  # video has n frames, mid point is (n-1)//2 as count starts with 0 中间点
        # 找到左侧可裁剪时长范围
